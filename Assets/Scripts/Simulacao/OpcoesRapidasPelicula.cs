...
public class OpcoesRapidasPelicula : TouchSystem, IDragHandler, IEndDragHandler
{
    [Range(1, 2, order = 1)]
    public int Index;
    public int width;
    ...
    private OpcoesRapidasPelicula[] Instances;
    ...
    private void Start()
    {
        ...
        initialPos = RT.localPosition;
        Instances = FindObjectsOfType<OpcoesRapidasPelicula>();
    ...
    protected override void Update()
    {
        ...
        {
            float VEL = 100;
            if (Mathf.Abs(RT.localPosition.y - (RefPos.y + Tamanho + height * (Index - PageControl.Instance.IsOpcoesRapidas))) <= VEL)
            {
                IsAdapting = false;
                RT.localPosition = new Vector3(RefPos.x, RefPos.y + Tamanho + height * (Index - PageControl.Instance.IsOpcoesRapidas), RefPos.z);
                initialPos = RT.localPosition;
            }
            else
            {
                if (RT.localPosition.y > RefPos.y + Tamanho + height * (Index - PageControl.Instance.IsOpcoesRapidas))
                {
                    RT.localPosition = new Vector3(RefPos.x, RT.localPosition.y - VEL, RefPos.z);
                }
                else if (RT.localPosition.y < RefPos.y + Tamanho + height * (Index - PageControl.Instance.IsOpcoesRapidas))
                {
                    RT.localPosition = new Vector3(RefPos.x, RT.localPosition.y + VEL, RefPos.z);
                }
            }
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        switch (Index)
        {
            case 1:
                if (RT.localPosition.y - Distance.y < 10 && RT.localPosition.y - Distance.y > -10)
                {
                    ...
                    {
                        PageControl.Instance.IsOpcoesRapidas = 1;
                    }
                    else if (RT.localPosition.y <= 0)
                    {
                        PageControl.Instance.IsOpcoesRapidas = 2;
                    }
                }
                else
                {
        ...
        foreach (OpcoesRapidasPelicula _pelicula in Instances)
        {
            _pelicula.Drop();
        }
